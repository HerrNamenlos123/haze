...
static nfdresult_t AddFiltersToDialog(IFileDialog *fileOpenDialog,
  ...
  fileOpenDialog->lpVtbl->SetFileTypes(fileOpenDialog, filterCount + 1,
                                       specList);
...
static nfdresult_t AllocPathSet(IShellItemArray *shellItems,
  ...
  for (DWORD i = 0; i < numShellItems; ++i) {
    ...
    SFGAOF attribs;
    result =
        shellItem->lpVtbl->GetAttributes(shellItem, SFGAO_FILESYSTEM, &attribs);
    if (!SUCCEEDED(result)) {
      NFDi_SetError(ERRORMSG);
      return NFD_ERROR;
    }
...
static nfdresult_t SetDefaultPath(IFileDialog *dialog,
  ...
  HRESULT result = SHCreateItemFromParsingName(
      defaultPathW, NULL, &IID_IShellItem, (void **)&folder);
...
nfdresult_t NFD_OpenDialog(const nfdchar_t *filterList,
  ...
  HRESULT result =
      CoCreateInstance(&CLSID_FileOpenDialog, NULL, CLSCTX_ALL,
                       &IID_IFileOpenDialog, (void **)(&fileOpenDialog));
  ...
  if (SUCCEEDED(result)) {
    ...
    result = shellItem->lpVtbl->GetDisplayName(shellItem, SIGDN_FILESYSPATH,
                                               &filePath);
    if (!SUCCEEDED(result)) {
...
nfdresult_t NFD_OpenDialogMultiple(const nfdchar_t *filterList,
  ...
  result = fileOpenDialog->lpVtbl->SetOptions(fileOpenDialog,
...
nfdresult_t NFD_SaveDialog(const nfdchar_t *filterList,
  ...
  HRESULT result =
      CoCreateInstance(&CLSID_FileSaveDialog, NULL, CLSCTX_ALL,
                       &IID_IFileSaveDialog, (void **)(&fileSaveDialog));
...
nfdresult_t NFD_PickFolder(const nfdchar_t *defaultPath, nfdchar_t **outPath) {
  ...
  HRESULT result =
      CoCreateInstance(&CLSID_FileOpenDialog, NULL, CLSCTX_ALL,
                       &IID_IFileOpenDialog, (void **)(&fileDialog));
  if (!SUCCEEDED(result)) {
  ...
  // folders
  if (!SUCCEEDED(fileDialog->lpVtbl->SetOptions(fileDialog,
  ...
  if (SUCCEEDED(result)) {
    ...
    wchar_t *path = NULL;
    result = shellItem->lpVtbl->GetDisplayName(
        shellItem, SIGDN_DESKTOPABSOLUTEPARSING, &path);
    if (!SUCCEEDED(result)) {
