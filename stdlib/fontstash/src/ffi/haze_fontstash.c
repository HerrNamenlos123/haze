...
hzstd_int_t haze_fontstash_add_font(void *ctx, hzstd_str_t name,
                                    hzstd_str_t path) {
  assert(ctx);
  return fonsAddFont((FONScontext *)ctx, HZSTD_CSTR(name), HZSTD_CSTR(path));
}

hzstd_int_t haze_fontstash_add_font_from_memory(void *ctx, hzstd_str_t name,
                                                hzstd_cptr_t data,
                                                hzstd_int_t length) {
  assert(ctx);
...
void haze_fontstash_layout_text(void *ctx, hzstd_int_t font, float size,
                                float x, float y, hzstd_str_t text,
...
haze_fontstash_atlas_t haze_fontstash_get_atlas(void *ctx) {
  ...
  const unsigned char *pixels =
      fonsGetTextureData((FONScontext *)ctx, &w, &hgt);
...
haze_fontstash_metrics_t haze_fontstash_get_metrics(hzstd_cptr_t ctx,
                                                    hzstd_int_t font,
                                                    hzstd_real_t size) {
  FONScontext *fs = ctx;
