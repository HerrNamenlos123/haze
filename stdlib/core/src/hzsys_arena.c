...
hzsys_arena_t *hzsys_arena_create(size_t chunk_size) {
  hzsys_arena_t *arena = (hzsys_arena_t *)hzsys_malloc_zeroed(chunk_size);
  if (!arena) {
...
hzsys_arena_chunk_t *hzsys_arena_create_chunk(size_t chunk_size) {
  size_t alloc_size = sizeof(hzsys_arena_chunk_t) + chunk_size;
  hzsys_arena_chunk_t *chunk =
      (hzsys_arena_chunk_t *)hzsys_malloc_zeroed(alloc_size);
  if (chunk == 0) {
...
hzsys_arena_chunk_t *hzsys_arena_enlarge(hzsys_arena_chunk_t *last_chunk,
                                         size_t chunk_size) {
...
void *hzsys_arena_allocate(hzsys_arena_t *arena, size_t size,
                           size_t alignment) {
...
void hzsys_attach_subarena(hzsys_arena_t *arena, hzsys_arena_t *subarena) {
  if (!arena->first_sub_arena) {
    arena->first_sub_arena = hzsys_arena_allocate(
        arena, sizeof(hzsys_subarena_entry_t), alignof(hzsys_subarena_entry_t));
    ...
    last_arena->next = hzsys_arena_allocate(
        arena, sizeof(hzsys_subarena_entry_t), alignof(hzsys_subarena_entry_t));
    last_arena->next->arena = subarena;
  }
}

hzsys_arena_t *hzsys_arena_create_subarena(hzsys_arena_t *arena) {
  hzsys_arena_t *subarena = hzsys_arena_create(HZSYS_DEFAULT_ARENA_CHUNK_SIZE);
...
void hzsys_detach_subarena(hzsys_arena_t *arena) {
  if (!arena->first_sub_arena) {
    ...
    if (arena->first_sub_arena->arena == arena) {
      arena->first_sub_arena = 0;
      return;
    }
  } else {                                        // Two or more actions
    if (arena->first_sub_arena->arena == arena) { // Remove first one
...
size_t hzsys_arena_register_cleanup_action(hzsys_arena_t *arena,
  ...
  if (!arena->cleanup_action) {
    arena->cleanup_action =
        hzsys_arena_allocate(arena, sizeof(hzsys_arena_cleanup_action_t),
                             alignof(hzsys_arena_cleanup_action_t));
