...
#define HZSTD_ALLOC_STRUCT(arena, struct_t, struct_ptr_t, value)               \
  ({                                                                           \
    struct_ptr_t tmp = hzstd_arena_allocate(                                   \
        (arena)->arenaImpl, sizeof(struct_t), alignof(struct_t));              \
    *tmp = value;                                                              \
    tmp;                                                                       \
  })
...
#define HZSTD_DESTROY_LOCAL_ARENA(arena)                                       \
  hzstd_arena_cleanup_and_free((arena)->arenaImpl)
