...
// checking and are called by the compiler for primitive arithmetic. The
// functions are STATIC + INLINE to make sure they are DUPLICATED in every
...
#define DEFINE_HZSTD_ARITHMETIC(type)                                          \
  ...
  hzstd_arithmetic_add_##type(type a, type b) {                                \
    type result;                                                               \
  ...
  static inline __attribute__((always_inline)) type                            \
  hzstd_arithmetic_sub_##type(type a, type b) {                                \
    type result;                                                               \
  ...
  static inline __attribute__((always_inline)) type                            \
  hzstd_arithmetic_mul_##type(type a, type b) {                                \
    type result;                                                               \
...
#define DEFINE_HZSTD_ARITHMETIC_UNSIGNED_DIV(type)                             \
  static inline __attribute__((always_inline)) type                            \
  hzstd_arithmetic_div_##type(type a, type b) {                                \
  ...
  static inline __attribute__((always_inline)) type                            \
  hzstd_arithmetic_mod_##type(type a, type b) {                                \
    if (__builtin_expect(b == 0, 0)) {                                         \
      hzstd_panic_fmt("Division by Zero");                                     \
    }                                                                          \
    return a % b;                                                              \
