...
#define HZSTD_DYNAMIC_ARRAY_PUSH(array, elem)                                                                          \
  ((void)hzstd_assert(hzstd_dynamic_array_push(array, &elem) == hzstd_dynamic_array_result_ok))
...
{
  hzstd_assert(da != NULL && out_elem != NULL);
  if (index >= da->size) {
    return hzstd_dynamic_array_result_out_of_bounds;
  }
...
{
  hzstd_assert(da != NULL && out_elem_ptr != NULL);
  if (index >= da->size) {
    return hzstd_dynamic_array_result_out_of_bounds;
  }
...
static inline size_t hzstd_dynamic_array_capacity(const hzstd_dynamic_array_t *da)
{
  return da ? da->capacity : 0;
}
static inline void *hzstd_dynamic_array_raw_buffer(const hzstd_dynamic_array_t *da)
{
  return da->buffer;
}
