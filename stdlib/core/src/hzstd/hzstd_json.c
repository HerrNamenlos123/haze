...
cJSON *hzstd_json_parse(hzstd_allocator_t allocator, hzstd_str_t data,
...
cJSON *hzstd_json_create_number(hzstd_allocator_t allocator,
                                hzstd_real_t data) {
  hzstd_json_use_arena(allocator);
...
double hzstd_json_get_number_value(hzstd_allocator_t allocator, cJSON *json) {
  return cJSON_GetNumberValue(json);
}

cJSON *hzstd_json_get_object_item(hzstd_allocator_t allocator, cJSON *json,
                                  hzstd_str_t name) {
  hzstd_json_use_arena(allocator);
  return cJSON_GetObjectItem(json, hzstd_cstr_from_str(allocator, name));
}

size_t hzstd_json_get_array_size(hzstd_allocator_t allocator, cJSON *json) {
  return cJSON_GetArraySize(json);
}

cJSON *hzstd_json_get_array_item(hzstd_allocator_t allocator, cJSON *json,
                                 size_t index) {
  hzstd_json_use_arena(allocator);
...
hzstd_bool_t hzstd_json_add_item_to_object(hzstd_allocator_t allocator,
                                           cJSON *object, hzstd_str_t name,
                                           cJSON *item) {
  return cJSON_AddItemToObject(object, hzstd_cstr_from_str(allocator, name),
                               item);
}

hzstd_bool_t hzstd_json_add_item_to_array(hzstd_allocator_t allocator,
                                          cJSON *object, cJSON *item) {
