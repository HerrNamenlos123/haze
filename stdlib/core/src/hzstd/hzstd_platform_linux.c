...
static void *hzstd_panic_handler_thread(void *_) {
  ...
  for (;;) {
    ...
    do { numberOfFrames++; } while (unw_step(&cursor) > 0);
...
static void hzstd_panic_handler(int sig, siginfo_t *si, void *ucontext) {
  ...
  default:
    store_panic_reason(
        HZSTD_STRING_FROM_CSTR("Segmentation Fault: Unknown Code"));
    panic_type = hzstd_panic_type_segfault;
    break;
  }
...
#else
  if (strerror_r(err, buf, sizeof(buf)) == 0) msg = buf;
  else                                         msg = "Unknown error";
#endif
  if (msg) {
    size_t len   = strlen(msg);
    char  *gc    = hzstd_allocate(hzstd_make_heap_allocator(), len + 1);
    if (gc) { memcpy(gc, msg, len); gc[len] = '\0'; out->stderr_data = gc; }
  }
}
...
int hzstd_spawn_process(hzstd_str_t exe, hzstd_str_t *argv, size_t argc,
  ...
  if (!inherit_stdio) {
    if (pipe(stdout_pipe) != 0) { process_set_error_message(out, errno); return errno; }
    if (pipe(stderr_pipe) != 0) {
      close(stdout_pipe[0]); close(stdout_pipe[1]);
      process_set_error_message(out, errno); return errno;
    }
  ...
  if (envp) {
    ...
    if (!envp_c) {
      for (size_t i = 0; argv_c[i]; ++i) free(argv_c[i]);
      free(argv_c);
      return ENOMEM;
    }
    for (size_t i = 0; i < envc; ++i) envp_c[i] = strdup(envp[i].data);
    envp_c[envc] = NULL;
  }

  char *cwd_c = cwd ? strdup(cwd->data) : NULL;
#ifdef __linux__
  if (cwd_c) posix_spawn_file_actions_addchdir_np(&actions, cwd_c);
#endif
  ...
  for (size_t i = 0; argv_c[i]; ++i) free(argv_c[i]);
  free(argv_c);
  if (envp_c) { for (size_t i = 0; i < envc; ++i) free(envp_c[i]); free(envp_c); }
  free(cwd_c);
  posix_spawn_file_actions_destroy(&actions);
  posix_spawnattr_destroy(&attrs);

  if (!inherit_stdio) { close(stdout_pipe[1]); close(stderr_pipe[1]); }

  if (rc != 0) { process_set_error_message(out, rc); return rc; }

  int status;
  if (waitpid(pid, &status, 0) < 0) { process_set_error_message(out, errno); return errno; }
