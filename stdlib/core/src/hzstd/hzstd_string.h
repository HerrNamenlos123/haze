...
// This is only possible for haze strings with known length, and not for c strings (cstr/ccstr)
#define HZSTD_STRING_GET_BYTE(str, index)                                                                              \
  ({                                                                                                                   \
    ...
    if (__hz_temp_index < 0 || __hz_temp_index >= __hz_temp_string.length) {                                           \
      HZSTD_PANIC_FMT(                                                                                                 \
          "string byte access index out of range [%ld] with length %lu", __hz_temp_index, __hz_temp_string.length);    \
    }                                                                                                                  \
