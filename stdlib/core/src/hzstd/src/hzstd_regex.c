...
void hzstd_regex_init_table(hzstd_regex_blob_t *table, size_t table_count) {
  ...
  for (size_t i = 0; i < table_count; ++i) {
    ...
    int rc = pcre2_serialize_decode((pcre2_code **)&table[i].code, 1,
                                    table[i].data, NULL);

    if (rc != 1 || table[i].code == NULL) {
      /* Static regex deserialization must never fail */
      hzstd_panic_fmt("Regex decoding failed. This should never happen. This "
                      "is purely a runtime bug and definitely "
...
hzstd_regex_runtime_compile(hzstd_str_t pattern, hzstd_str_t flags,
  ...
  pcre2_code *code = pcre2_compile((PCRE2_SPTR)pattern.data, pattern.length,
...
hzstd_bool_t hzstd_regex_match(hzstd_regex_t regex, hzstd_str_t text) {
  ...
  int rc =
      pcre2_match(code, (PCRE2_SPTR)text.data, text.length, 0, 0, md, NULL);

  pcre2_match_data_free(md);
  return rc >= 0;
}

hzstd_regex_find_one_result_t hzstd_regex_find(hzstd_allocator_t allocator,
                                               hzstd_regex_t regex,
                                               hzstd_str_t text) {
...
static void sb_append(hzstd_sb_t *sb, const char *data, size_t len) {
  if (sb->len + len + 1 > sb->cap) {
    ...
    if (!sb->data) {
      hzstd_panic_fmt("System out of memory error while allocating buffer for "
                      "Regex replacement");
    }
    sb->cap = new_cap;
  }
...
hzstd_str_t hzstd_regex_replace(hzstd_regex_t regex, hzstd_str_t text,
                                hzstd_str_t replacement) {
  if (!regex.blob || !regex.blob->code) {
    return text;
  }
  ...
  while (1) {
    int rc = pcre2_match(code, (PCRE2_SPTR)text.data, text.length, offset, 0,
                         md, NULL);
...
hzstd_regex_find_all(hzstd_allocator_t allocator, hzstd_regex_t regex,
                     hzstd_str_t text) {
  HZSTD_DARRAY(hzstd_regex_find_match_t)
