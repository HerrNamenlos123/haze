...
hzstd_arena_t *hzstd_arena_create() {
  hzstd_arena_t *arena =
      (hzstd_arena_t *)hzstd_heap_allocate(sizeof(hzstd_arena_t));
  return arena;
}
...
static hzstd_arena_chunk_t *hzstd_arena_create_chunk(size_t chunk_size) {
  ...
  hzstd_arena_chunk_t *chunk =
      (hzstd_arena_chunk_t *)hzstd_heap_allocate(alloc_size);
...
void *hzstd_arena_allocate(hzstd_arena_t *arena, size_t size) {
  ...
  size_t chunk_size =
      HZSTD_MAX(HZSTD_DEFAULT_ARENA_CHUNK_SIZE, size + alignment);
