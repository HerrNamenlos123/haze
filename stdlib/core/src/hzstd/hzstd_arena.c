...
{
  hzstd_arena_t* arena = (hzstd_arena_t*)hzstd_malloc_zeroed(sizeof(hzstd_arena_t));
  return arena;
}
...
void hzstd_arena_deregister_cleanup_action(hzstd_arena_t* arena, size_t actionId)
{
  ...
  // TODO: Fix this with string concatenation
  // hzstd_panic("Arena Cleanup action with id %llu is not registered", actionId);
  hzstd_panic("Arena Cleanup action with id <> is not registered");
}
