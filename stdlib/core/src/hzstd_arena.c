...
{
  hzstd_arena_t* arena = (hzstd_arena_t*)hzstd_malloc_zeroed(initial_chunk_size);
  if (!arena) {
    hzstd_panic("System Out Of Memory: Arena control structure allocation failed");
  }
  return arena;
}
...
static hzstd_arena_chunk_t* hzstd_arena_create_chunk(size_t chunk_size)
{
  ...
  hzstd_arena_chunk_t* chunk = (hzstd_arena_chunk_t*)hzstd_malloc_zeroed(alloc_size);
  if (chunk == 0) {
    hzstd_panic("System Out Of Memory: Arena chunk allocation of size %llu failed", alloc_size);
  }
...
void hzstd_attach_subarena(hzstd_arena_t* arena, hzstd_arena_t* subarena)
{
  if (!arena->first_sub_arena) {
    arena->first_sub_arena
        = hzstd_arena_allocate(arena, sizeof(hzstd_subarena_entry_t), alignof(hzstd_subarena_entry_t));
    ...
    last_arena->next = hzstd_arena_allocate(arena, sizeof(hzstd_subarena_entry_t), alignof(hzstd_subarena_entry_t));
...
{
  hzstd_arena_t* subarena = hzstd_arena_create(initial_chunk_size);
...
size_t hzstd_arena_register_cleanup_action(hzstd_arena_t* arena, void (*action)(void* actiondata), void* actiondata)
{
  ...
  if (!arena->cleanup_action) {
    arena->cleanup_action
        = hzstd_arena_allocate(arena, sizeof(hzstd_arena_cleanup_action_t), alignof(hzstd_arena_cleanup_action_t));
