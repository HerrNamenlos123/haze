...
{
  return hzstd_arena_allocate(hzstd_json_current_arena, size, alignof(hzstd_cptr_t));
}
...
cJSON* hzstd_json_parse(hzstd_arena_t* arena, hzstd_str_t data, hzstd_str_ref_t* error)
{
  ...
  if (!cjson) {
    error->data = hzstd_str_from_cstr_dup(arena, (hzstd_cstr_t)cJSON_GetErrorPtr());
    return 0;
  }
...
{
  hzstd_arena_t* arena = hzstd_arena_create(HZSTD_DEFAULT_ARENA_CHUNK_SIZE);
...
hzstd_str_ref_t* hzstd_json_get_string_value(hzstd_arena_t* arena, cJSON* json)
{
  ...
  hzstd_str_ref_t* result = hzstd_arena_allocate(arena, sizeof(hzstd_str_ref_t), alignof(hzstd_str_ref_t));
  result->data = hzstd_str_from_cstr_ref(value);
  return result;
}

double hzstd_json_get_number_value(hzstd_arena_t* arena, cJSON* json) { return cJSON_GetNumberValue(json); }
...
size_t hzstd_json_get_array_size(hzstd_arena_t* arena, cJSON* json) { return cJSON_GetArraySize(json); }

cJSON* hzstd_json_get_array_item(hzstd_arena_t* arena, cJSON* json, size_t index)
{
  hzstd_json_use_arena(arena);
