...
#define CLAY_STRING(string)                                                                                            \
  (CLAY__INIT(Clay_String) { .length = CLAY__STRING_LENGTH(CLAY__ENSURE_STRING_LITERAL(string)), .chars = (string) })

#define CLAY_STRING_CONST(string)                                                                                      \
  { .length = CLAY__STRING_LENGTH(CLAY__ENSURE_STRING_LITERAL(string)), .chars = (string) }
...
// Similar behaviour can be achieved on an individual text element level by using
// Clay_TextElementConfig.hashStringContents
  ...
  while (end < text->length) {
    ...
    if (current == ' ' || current == '\n') {
      ...
      if (current == '\n') {
        ...
        previousWord = Clay__AddMeasuredWord(
            CLAY__INIT(Clay__MeasuredWord) { .startOffset = end + 1, .length = 0, .width = 0, .next = -1 },
            previousWord);
        lineWidth += dimensions.width;
...
void Clay__CloseElement(void)
{
  ...
  if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
    ...
    for (int32_t i = 0; i < openLayoutElement->childrenOrTextContent.children.length; i++) {
      int32_t childIndex
          = Clay__int32_tArray_GetValue(&context->layoutElementChildrenBuffer,
...
void Clay__OpenTextElement(Clay_String text, Clay_TextElementConfig* textConfig)
{
  ...
  textElement->minDimensions = CLAY__INIT(Clay_Dimensions) {
    .width = textMeasured->unwrappedDimensions.height, .height = textDimensions.height
  }; // TODO not sure this is the best way to decide min width for text
  ...
  textElement->elementConfigs = CLAY__INIT(Clay__ElementConfigArraySlice) {
    .length = 1,
    .internalArray
    = Clay__ElementConfigArray_Add(&context->elementConfigs,
...
void Clay__ConfigureOpenElementPtr(const Clay_ElementDeclaration* declaration)
{
  ...
  if (!Clay__MemCmp(
          (char*)(&declaration->cornerRadius), (char*)(&Clay__CornerRadius_DEFAULT), sizeof(Clay_CornerRadius))) {
    if (sharedConfig) {
      ...
      if (declaration->floating.attachTo == CLAY_ATTACH_TO_PARENT) {
        ...
        if (!parentItem) {
          context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData) {
              ...
              .errorText
              = CLAY_STRING("A floating element was declared with a parentId, but no element with that ID was found."),
              .userData = context->errorHandler.userData });
        }
        else {
  ...
  if (declaration->custom.customData) {
    Clay__AttachElementConfig(
        CLAY__INIT(Clay_ElementConfigUnion) { .customElementConfig
  ...
  if (declaration->scroll.horizontal | declaration->scroll.vertical) {
    Clay__AttachElementConfig(
        CLAY__INIT(Clay_ElementConfigUnion) { .scrollElementConfig
  ...
  if (!Clay__MemCmp(
          (char*)(&declaration->border.width), (char*)(&Clay__BorderWidth_DEFAULT), sizeof(Clay_BorderWidth))) {
    Clay__AttachElementConfig(
        CLAY__INIT(Clay_ElementConfigUnion) { .borderElementConfig
    ...
    for (int32_t i = 0; i < bfsBuffer.length; ++i) {
      ...
      if (sizingAlongAxis) {
        ...
        for (int32_t childOffset = 0; childOffset < resizableContainerBuffer.length; childOffset++) {
          ...
          // If we're laying out the children of a scroll panel, grow containers expand to the height of the inner
...
void Clay__CalculateFinalLayout(void)
{
  ...
  for (int32_t rootIndex = 0; rootIndex < context->layoutElementTreeRoots.length; ++rootIndex) {
    ...
    Clay__LayoutElementTreeNodeArray_Add(
        &dfsBuffer,
        CLAY__INIT(Clay__LayoutElementTreeNode) { .layoutElement = rootElement,
          ...
          if (!Clay__ElementIsOffscreen(&currentElementBoundingBox)) {
            ...
            if (borderConfig->width.betweenChildren > 0 && borderConfig->color.a > 0) {
              ...
              Clay_Vector2 borderOffset
                  = { (float)layoutConfig->padding.left - halfGap, (float)layoutConfig->padding.top - halfGap };
              if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
                for (int32_t i = 0; i < currentElement->childrenOrTextContent.children.length; ++i) {
                  Clay_LayoutElement* childElement = Clay_LayoutElementArray_Get(
                      &context->layoutElements, currentElement->childrenOrTextContent.children.elements[i]);
        ...
        for (int32_t i = 0; i < currentElement->childrenOrTextContent.children.length; ++i) {
          ...
          if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
            ...
            float whiteSpaceAroundChild = currentElement->dimensions.width
                - (float)(layoutConfig->padding.left + layoutConfig->padding.right) - childElement->dimensions.width;
            switch (layoutConfig->childAlignment.x) {
            case CLAY_ALIGN_X_LEFT:
              break;
            case CLAY_ALIGN_X_CENTER:
         ...
         .border = { .color = CLAY__DEBUGVIEW_COLOR_3, .width = { .bottom = 1 } } })
  {
           ...
           .scroll = { .horizontal = true, .vertical = true } })
    {
                                                                                       ...
                                                                                       : CLAY__DEBUGVIEW_COLOR_1 })
      {
        ...
        CLAY({ .layout
               = { .sizing = { .width = CLAY_SIZING_FIXED(contentWidth) }, .layoutDirection = CLAY_TOP_TO_BOTTOM } })
        {
        }
    ...
    if (context->debugSelectedElementId != 0) {
      ...
      CLAY({ .layout
             = { .sizing = { CLAY_SIZING_GROW(0), CLAY_SIZING_FIXED(300) }, .layoutDirection = CLAY_TOP_TO_BOTTOM },
             .backgroundColor = CLAY__DEBUGVIEW_COLOR_2,
...
void Clay_BeginLayout(void)
{
  ...
  Clay__ConfigureOpenElement(CLAY__INIT(Clay_ElementDeclaration) {
      ...
      .layout
      = { .sizing = { CLAY_SIZING_FIXED((rootDimensions.width)), CLAY_SIZING_FIXED(rootDimensions.height) } } });
  ...
  for (int32_t i = 0; i < context->scrollContainerDatas.length; ++i) {
    ...
    if (scrollContainerData->elementId == id.id) {
      return CLAY__INIT(Clay_ScrollContainerData) {
        ...
        = { scrollContainerData->boundingBox.width, scrollContainerData->boundingBox.height },
        .contentDimensions = scrollContainerData->contentSize,
        .config = *Clay__FindElementConfigWithType(scrollContainerData->layoutElement, CLAY__ELEMENT_CONFIG_TYPE_SCROLL)
                       .scrollElementConfig,
        .found = true
      };
    }
