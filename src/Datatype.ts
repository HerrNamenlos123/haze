...
export namespace Types {
  function promoteInteger(
    ...
    const a = module.parsedStore
      .getSymbolOrFail(aId)
      .asDatatypeSymbol()
      .type.asPrimitive();
    const b = module.parsedStore
      .getSymbolOrFail(bId)
      .asDatatypeSymbol()
      .type.asPrimitive();

    const getWiderInteger = (a: Datatype.Primitive, b: Datatype.Primitive) => {
      const aBits = a.getIntegerBits();
      const bBits = b.getIntegerBits();
      return aBits > bBits ? a : b;
    };

    const widenInteger = (a: Datatype.Primitive, b: Datatype.Primitive) => {
      if (a.primitive == b.primitive) {
        return a;
      }

      let sizeA = a.getIntegerBits();
      let sizeB = b.getIntegerBits();
      ...
      if (b.isUnsignedInteger() && !a.isUnsignedInteger()) {
        if (sizeB > sizeA) {
  ...
  export function getIntegerBinaryResult(
    ...
    return promoteInteger(module, aId, bId);
  }
}
