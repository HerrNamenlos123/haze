...
export namespace Conversion {
  ...
  // ── Comparability ─────────────────────────────────────────────────────────
  //
  // Whether `a == b` / `a != b` exists for a type at all. Equality in Haze is
  // VALUE-based throughout: a comparison either dispatches to a user-defined
  // `operator==`/`operator!=`, or is autogenerated member-by-member and
  // recurses (so a nested member with its own operator still uses it). There is
  // no identity comparison -- comparing two references compares what they point
  // AT, not where they point. (Identity is a separate concept and is not
  // implemented yet.)
  //
  // A type is comparable when:
  //   - primitive (except cptr, which has no safe comparison), or enum
  //   - a callable: compared as {fn, env}
  //   - a fixed array whose element type is comparable (its length is part of
  //     the type, so the comparison is a bounded, fully unrolled walk). A
  //     DYNAMIC array is not: its length is runtime state, so element-wise
  //     equality is a policy decision the language does not make for you --
  //     write an operator if you want it (see tabs.TabsArgs for one).
  //   - a union all of whose members are comparable
  //   - a struct that either declares one of the two operators, or is not
  //     opaque and has only comparable members
  //
  // Two deliberate exclusions:
  //   - OPAQUE structs never get autogenerated equality. Their members are not
  //     part of their contract, so comparing them memberwise would compare
  //     things the author chose not to expose.
  //   - SELF-REFERENTIAL types (a struct that can reach its own type through a
  //     member) never get it either. `inProgress` catches exactly that: on
  //     re-entry the answer is "not comparable", which propagates out to every
  //     enclosing type. This is what keeps an autogenerated comparison from
  //     recursing forever on a cyclic object graph -- for a linked list or a
  //     tree node, write the operator and decide the termination rule yourself.
  export function structDeclaresComparisonOperator(
    sr: Semantic.Context,
    structDef: Semantic.StructDatatypeDef
  ): boolean {
    const original = sr.cc.typeDefNodes.get(
      structDef.originalCollectedDefinition
    );
    if (original.variant !== Collect.ENode.StructTypeDef) {
      return false;
    }
    const structScope = sr.cc.scopeNodes.get(original.lexicalScope);
    if (structScope.variant !== Collect.ENode.StructLexicalScope) {
      return false;
    }
    return [...structScope.symbols].some((mId) => {
      const m = sr.cc.symbolNodes.get(mId);
      return (
        m.variant === Collect.ENode.FunctionOverloadGroupSymbol &&
        (m.overloadedOperator === EOverloadedOperator.Equal ||
          m.overloadedOperator === EOverloadedOperator.NotEqual)
      );
    });
  }

  export function typeSupportsComparison(
    sr: Semantic.Context,
    typeUseId: Semantic.TypeUseId,
    inProgress: Set<Semantic.TypeDefId> = new Set()
  ): boolean {
    const use = sr.typeUseNodes.get(sr.e.resolveAlias(typeUseId));
    const def = sr.typeDefNodes.get(use.type);

    switch (def.variant) {
      case Semantic.ENode.PrimitiveDatatype:
        // cptr is an opaque address with no meaning the language can compare.
        return def.primitive !== EPrimitive.cptr;
      ...
      case Semantic.ENode.FixedArrayDatatype:
        return typeSupportsComparison(sr, def.datatype, inProgress);
      ...
      case Semantic.ENode.TaggedUnionDatatype:
        return def.members.every((m) =>
          typeSupportsComparison(sr, m.type, inProgress)
        );

      case Semantic.ENode.UntaggedUnionDatatype:
        return def.members.every((m) =>
          typeSupportsComparison(sr, m, inProgress)
        );

      case Semantic.ENode.StructDatatype: {
        // An explicit operator answers the question outright, and does so even
        // for opaque and self-referential types -- the author wrote it, so they
        // have decided what equality means and where it stops.
        if (structDeclaresComparisonOperator(sr, def)) {
          return true;
        }
        if (def.opaque) {
          return false;
        }
        if (inProgress.has(use.type)) {
          return false;
        }
        inProgress.add(use.type);
        const allComparable = def.members.every((mId) => {
          const m = sr.symbolNodes.get(mId);
          if (m.variant !== Semantic.ENode.VariableSymbol || !m.type) {
            return false;
          }
          return typeSupportsComparison(sr, m.type, inProgress);
        });
        inProgress.delete(use.type);
  ...
  // Narrower question than typeSupportsComparison: may the compiler GENERATE a
  // comparison for this type? Everything above applies, plus one more rule --
  // a struct that declares either operator gets neither generated. The two are
  // autogenerated as a pair or not at all, so a type that defines only
  // `operator!=` deliberately has no `==`: writing one of them is a statement
  // that this type's equality is not memberwise, and silently generating the
  // other half would contradict it.
  export function typeSupportsAutogeneratedComparison(
    sr: Semantic.Context,
    typeUseId: Semantic.TypeUseId
  ): boolean {
    const use = sr.typeUseNodes.get(sr.e.resolveAlias(typeUseId));
    ...
    if (
      ...
      structDeclaresComparisonOperator(sr, def)
    ) {
      return false;
    }
    return typeSupportsComparison(sr, typeUseId);
  }
  ...
  export function makeComparisonResultType(
    ...
    // Composite types compare BY VALUE, whatever storage class they are held
    // in: a value, a `ref` and a `stackref` of the same type all compare what
    // they hold, never where they point. (Reference identity used to live here
    // -- two refs were equal iff they were the same machine pointer. It is
    // gone: identity is a separate concept and will get its own spelling.)
    //
    // The comparison itself is not built here; Elaborate expands it member by
    // member (compareElaboratedExprs), which is what lets each member use its
    // own operator when it has one. This only answers "does the operation
    // exist", and its answer must agree with what Elaborate is willing to
    // expand -- hence the same predicate on both sides.
    if (
      leftTypeId === rightTypeId &&
      typeSupportsAutogeneratedComparison(sr, leftTypeUseId)
    ) {
      return makePrimitiveAvailable(
        sr,
        EPrimitive.bool,
