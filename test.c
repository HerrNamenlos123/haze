...
static uint32_t wayland_wl_registry_bind(int fd,
                                         uint32_t registry,
                                         uint32_t name,
                                         char* interface,
                                         ...
                                         uint32_t version)
{
  ...
  uint16_t msg_announced_size = wayland_header_size + sizeof(name) + sizeof(interface_len) + roundup_4(interface_len)
...
static void wayland_handle_message(int fd, state_t* state, char** msg, uint64_t* msg_len)
{
  ...
  if (object_id == state->wl_registry && opcode == wayland_wl_registry_event_global) {
    ...
    assert(announced_size
           == sizeof(object_id) + sizeof(announced_size) + sizeof(opcode) + sizeof(name) + sizeof(interface_len)
...
void registry_global_handler(void* data,
...
const struct wl_registry_listener registry_listener
    = { .global = registry_global_handler, .global_remove = registry_global_remove_handler };
